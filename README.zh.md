...
├── 是 → 你的痛点是 skill 维护吗?
│       ├── 是 → 丰容 ✅
│       └── 否 (你想要多平台/长期记忆) → 你其实需要 hermes-agent
│
└── 否 → 你想要自己的 AI agent 吗?
        ├── 是 (愿意装一个完整系统) → hermes-agent ✅
        └── 否 (只是想偶尔用) → 都不需要
```

> **一句话**:丰容是"**外科医生**"——精确修复既有 skill;hermes-agent 是"**长期助理**"——从经验中慢慢成长。两者**互补而非竞争**。
...
| 论文 Figure 1(冰水/热水示例) | **未复用**——本技能的 6 个 examples 全部基于 Claude Code 工具链原创 |

**直接来源说明**:四类反思框架、`S_body`/`S_app` 结构、反思记录字段、分类逻辑(成功路径/失败路径)、字面对比思路均由论文提供。

**本仓库的工程化贡献**:(1) graphviz 决策树(将论文决策表结构化);(2) Anti-patterns/Red Flags/Common Mistakes/Self-Test 等运维检查项;(3) Quick Algorithm 伪代码;(4) 6 个基于 Claude Code 的原创 examples;(5) reflection-record 模板。详细归因见 [`SKILL.md - 学术来源声明`](./SKILL.md#-学术来源声明-attribution)。
